...
Instead, we need to use a projected CRS that is designed for a smaller region within which our dataset is located. In the US we commonly use a series of projected CRSs called UTM Zones which are based on the NAD83 datum and the Universal Transverse Mercator projection, for example **NAD83 / UTM Zone 16** (EPSG: 26916) covers Chicagoe. Another common set of CRS definitions in the US is the State Plane series. Chicago sits within **NAD83 / Illinois East (ftUS)** (EPSG:3435).
...
5. Use the CRS dropdown to select `NAD83 / Illinois East (ftUS)`.
    - If this CRS is not in the dropdown, use Select CRS button to open a more detailed dialog box.
