...
- Open the file `chicagotracts.shp` as a new vector layer by navigating to Layer -> Add Layer -> Add Vector Layer, or through your local or network drives in the Browser panel.  ** make this part more clear; diff versions may be different)
...
**Change Projections in QGIS**
 ...
 Using the same Chicago census tract shapefile as before, inspect the current projection. 
...
Notice how the shape of the city changed, now that the projections are not in agreement. Inspect the projection for the census tract layer by right clicking on the layer and going to Layer CRS. The layer is not projected, instead using the CRS WGS84. Since the project is now in EPSG:26916, however, you should see the option Set to EPSG:26916. Now you can see the city's shape has changed again with the projections being back in agreement. 
:::
